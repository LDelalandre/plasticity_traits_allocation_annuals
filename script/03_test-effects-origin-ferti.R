library(tidyverse)
source("script/01_import-data.R") # load t2_traits
...
for (ftrait in FTRAITS){
  ...
  EM <- emmeans::emmeans(mod_effect_ferti_origin, specs = c("fertilization"),  type = "response",
                         adjust = "tuckey") # emmeans version 1.5.2-1

  library(multcomp)
  posthoc <- summary(glht(mod_effect_ferti_origin, linfct = mcp(fertilization = "Tukey")), test = adjusted("fdr"))
  ...
  variance <- MuMIn::r.squaredGLMM(mod_effect_ferti_origin) %>% 
    round(digits = 2)
